...
MAX_W      = 7.5              # THz — full DOS range (auto-trimmed in ROOT plot)
...
def gaussian_dos(phonon_freqs_thz, W, sigma):
    ...
    # Normalize by number of q-points (consistent with ROOT /17576)
...
ax.axvline(x=1.0, color='red', linewidth=1.0,
           linestyle=':', alpha=0.6)
freq_max_plot = freqs_thz[freqs_thz > 0].max() + 0.3
ax.set_xlim(MIN_W, freq_max_plot)
...
# meV axis on top (1 THz = 4.14 meV)
ax_top = ax.twiny()
ax_top.set_xlim(MIN_W * 4.14, freq_max_plot * 4.14)
ax_top.set_xlabel("Energy (meV)", fontsize=11)
...
with open(OUTPUT_TXT, 'w', newline='') as f:
    ...
    # NO header — ROOT reads plain columns: Freq Dos_1 Dos_2 Dos_3
    # DOS already normalized by nq (consistent with ROOT /17576)
    for i in range(len(W)):
        row = [f'{W[i]:.6f}'] + \
              [f'{branch_dos[ib][i]:.8f}' for ib in range(nbnd)]
        writer.writerow(row)

print(f"  Data saved: {OUTPUT_TXT}")

# ================================================================
#  SAVE BRANCH DOS — one file per branch + total
#  Format: Freq(THz)  DOS   — readable directly by ROOT TGraph
# ================================================================
for ib in range(nbnd):
    label    = ['TA1','TA2','LA'][ib] if ib < 3 else f'branch_{ib+1}'
    out_file = OUTPUT_TXT.replace('.txt', f'_{label}.txt')
    with open(out_file, 'w') as f:
        for i in range(len(W)):
            f.write(f"{W[i]:.6f}\t{branch_dos[ib][i]:.8f}\n")
    print(f"  Saved {label:>4} DOS : {out_file}")

# Save total DOS
out_total = OUTPUT_TXT.replace('.txt', '_Total.txt')
with open(out_total, 'w') as f:
    for i in range(len(W)):
        f.write(f"{W[i]:.6f}\t{total_dos[i]:.8f}\n")
print(f"  Saved Total DOS : {out_total}")
...
FIT_MIN   = 0.8    # THz — fit start (matches ROOT pol3 range)
FIT_MAX   = 2.0    # THz — fit end   (matches ROOT pol3 range)
EVAL_AT   = 1.0    # THz — evaluate fitted polynomial here
FIT_DEG   = 3      # polynomial degree (3 = cubic, matches ROOT pol3)
...
for ib in range(nbnd):
    ...
    # Polynomial fit degree FIT_DEG (cubic=3, matches ROOT pol3)
    coeffs    = np.polyfit(W_v, F_v, FIT_DEG)
    fit_func  = np.poly1d(coeffs)
    slope     = coeffs[-2]   # linear coefficient
    intercept = coeffs[-1]   # constant term
