...
print(f"  Max frequency : {freqs.max():.2f} cm^-1")
print(f"  Min frequency : {freqs.min():.2f} cm^-1")
n_imag = np.sum(freqs < -5.0)
if n_imag > 0:
    print(f"  WARNING: {n_imag} imaginary frequencies")
else:
...
fig.suptitle("Phonon Dispersion of Nb (BCC)\nPBE | GBRV-USPP | 12×12×12 k-mesh | 2×2×2 q-mesh",
             fontsize=12, fontweight='bold')

ax = ax_disp
for ib in range(nbnd):
    ax.plot(q_dist, freqs[:, ib], '-',
...
ax.set_ylim(bottom=min(-15, freqs.min()-10),
            top=freqs.max()+25)
...
imax = np.unravel_index(np.argmax(freqs), freqs.shape)
ax.annotate(f'{freqs.max():.0f} cm$^{{-1}}$',
            xy=(q_dist[imax[0]], freqs.max()),
...
# Frequency table
print("\n  Frequencies at high-symmetry points (cm^-1):")
print(f"  {'Point':<6} {'Mode 1':>10} {'Mode 2':>10} {'Mode 3':>10}")
print("  " + "-"*38)
for idx, lab in zip(seg_ends, labels_raw):
    if idx < len(q_dist):
        f = freqs[idx]
        print(f"  {lab:<6} {f[0]:>10.2f} {f[1]:>10.2f} {f[2]:>10.2f}")
